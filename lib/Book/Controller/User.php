...
class Book_Controller_User extends Zikula_AbstractController {
    ...
    public function main() {
        ...
        $render->assign('book_names', $bookTitleArray);
    ...
    public function toc($args) {
        ...
        foreach ($chapters as $chapter_item) {
            ...
            if ($chapter_item['chap_number'] > 0) {
                $articles = ModUtil::apiFunc('Book', 'user', 'getallarticles', array('chap_id' => $chap_id, 'get_content' => false));
                $art_array = array();
...
// Please acknowledge use of this code by including this header.
    ...
    public function display($args) {
        ...
        $render->assign('chapter', $chapter['chap_name']);
    ...
    public function displayarticle($args) {
        ...
        $chapter = ModUtil::apiFunc('Book', 'user', 'getchapter', array('chap_id' => $article['chap_id']));
        ...
        if ($article['next'] == 0) {
            ...
            $next_art = ModUtil::apiFunc('Book', 'user', 'getarticlebyartnumber', array('art_number' => $article['art_number'] + 1, 'chap_id' => $article['chap_id']));
            $article['next'] = $next_art['art_id'];
        }
        if (($article['prev'] == 0) || ($article['art_numner'] != 1)) {
            //no link assigned, lets see if we can find the next one
            $prev_art = ModUtil::apiFunc('Book', 'user', 'getarticlebyartnumber', array('art_number' => $article['art_number'] - 1, 'chap_id' => $article['chap_id']));
            $article['prev'] = $prev_art['art_id'];
        }
        $render->assign('art_id', $article['art_id']);
        $render->assign('chap_id', $article['chap_id']);
        $render->assign('section_id', $article['art_number']);
        $render->assign('chap_number', $chapter['chap_number']);
        ...
        if (!ModUtil::apiFunc('Book', 'user', 'setcounter', array('art_id' => $art_id, 'counter' => $article['counter']))) {
...
//code for exporting the chapters.
    ...
    public function displayfigure($args) {
        ...
        //get the chapter title
        $figure = ModUtil::apiFunc('Book', 'user', 'getfigure', array('fig_number' => $fig_number, 'chap_number' => $chap_number, 'book_id' => $book_id));
        //permission check
        //$book = ModUtil::apiFunc('Book', 'user', 'get', array('book_id' => $figure['book_id']));
        ...
        if ($figure['fig_perm'] != 0) {
            $visible_link = $this->buildlink($figure['img_link'], $figure['fig_title'], $width, $height, true, false, true, $stand_alone);
        } else {
            $visible_link = __("This figure cannot be displayed because permission has not been granted yet.");
        }
        //rint $visible_link;
        if ($figure['fig_content'] === "") {
            $render->assign('fig_content_empty', "true");
        } else {
            $render->assign('fig_content_empty', "false");
        }
...
//grab all the articles and this time we do need the content
        ...
        $render->assign('chapter_title', $chapter['chap_name']);
        $render->assign('chapter_number', $chapter['chap_number']);
        ...
        foreach ($articles as $article_item) {
            ...
            if (!ModUtil::apiFunc('Book', 'user', 'setcounter', array('art_id' => $article_item['art_id'], 'counter' => $article_item['counter']))) {
                ...
                if (pnUserLoggedIn()) {
                    ...
                    if (count($items) < 11) {
                        ...
                        if (!$ispresent) {
                            ...
                            $gloss_id = ModUtil::apiFunc('Book', 'admin', 'createglossary', array('term' => $term, 'definition' => "", 'URL' => $URL, 'user' => $user));
...
//walk through each highlight and get the important information
        ...
        foreach ($highlights as $hItem) {
            //grab each article
            $article = ModUtil::apiFunc('Book', 'user', 'getarticle', array('art_id' => $hItem['art_id']));
...
//grab the article to find the offsets
        $art_array = ModUtil::apiFunc('Book', 'user', 'getarticle', array('art_id' => $art_id));
