...
function book_adminapi_create($args) {
// Argument check
...
// item
    pnModCallHooks('item', 'create', $arg['book_id'], 'book_id');
 ...
 */
function book_adminapi_createchapter($args) {
// Argument check
...
//with the same number
        $args['number'] = pnModAPIFunc(	'Book',
                'user',
                'countchapters',
                array('book_id' => $book)) + 1;
    }
// Security check - important to do this as early on as possible to
// avoid potential security holes or just too much wasted processing
    if (!SecurityUtil::checkPermission('Book::', "::", ACCESS_ADD)) {
...
// Let any hooks know that we have created a new item.
    pnModCallHooks('item', 'create', $args['chap_id'], 'chap_id');
 ...
 * @return book item ID on success, false on failure
 */
function book_adminapi_createarticle($args) {
// Argument check
...
//negative chapters
    ...
    //add glossary terms
   $contents = book_adminapi_add_glossary_terms(array('in_text' => $args['contents']));
...
// Let any hooks know that we have created a new item.
    pnModCallHooks('item', 'create', $args['art_id'], 'art_id');

// Return the id of the newly created item to the calling process
    return $args['art_id'];
}

function book_adminapi_createfigure($args) {
// Argument check
//we make sure that all the arguments are there
//and that the chapter is larger than 1
//negative chapters
...
// Let any hooks know that we have created a new item.
    pnModCallHooks('item', 'create', $args['fig_id'], 'fig_id');

// Return the id of the newly created item to the calling process
    return $args['fig_id'];
}

function book_adminapi_createglossary($args) {
// Argument check
    if ((!isset($args['term'])) ||
...
// argument that all of the other functions use to reference this
// item
    pnModCallHooks('item', 'create', $gloss_id, 'gloss_id');
 ...
 */
function book_adminapi_delete($args) {
// Argument check
...
// The user API function is called.
    $item = pnModAPIFunc('Book',
            'user',
            'get',
            array('book_id' => $args['book_id']));
...
// delete hook we're not passing any extra info
    pnModCallHooks('item', 'delete', $args['book_id'], '');
...
function book_adminapi_deletechapter($args) {
...
// Let any hooks know that we have deleted an item.  As this is a
// delete hook we're not passing any extra info
    pnModCallHooks('item', 'deletechapter', $chap_id, '');

// Let the calling process know that we have finished successfully
    return true;
}

function book_adminapi_deletearticle($args) {
// Argument check
    if (!isset($args['art_id'])) {
        LogUtil::registerArgsError();
        return false;
...
// Security check
    ...
    pnModCallHooks('item', 'deletearticle', $args['art_id'], '');
...
function book_adminapi_deletefigure($args) {
// Argument check
...
//security check
    ...
    pnModCallHooks('item', 'deletefigure', $args['fig_id'], '');

// Let the calling process know that we have finished successfully
    return true;
}

function book_adminapi_deleteglossary($args) {
// Argument check
...
//security check
    ...
    pnModCallHooks('item', 'deleteglossary', $args['gloss_id'], '');
 ...
 */
function book_adminapi_update($args) {
// Argument check
    if ((!isset($args['book_id'])) ||
...
function book_adminapi_updatechapter($args) {
// Argument check
    if ((!isset($args['book_id'])) ||
 ...
 */
function book_adminapi_updatearticle($args) {
    // Argument check
    if ((!isset($args['book_id'])) ||
            (!isset($args['art_id'])) ||
            (!isset($args['title'])) ||
...
function book_adminapi_updatefigure($args) {
// Argument check
//we make sure that all the arguments are there
//and that the chapter is larger than 1
//negative chapters
...
// Let any hooks know that we have created a new item.
    pnModCallHooks('book', 'updatefigure', $fig_id, 'fig_id');

    return true;
}

function book_adminapi_updateglossary($args) {
// Argument check
//we make sure that all the arguments are there
    if ((!isset($args['gloss_id'])) ||
            (!isset($args['term'])) ||
...
//You must have general edit ability to modify the glossary
    ...
    // Let any hooks know that we have created a new item.
    pnModCallHooks('book', 'updateglossary', $args['gloss_id'], 'gloss_id');

    return true;
}


function book_adminapi_createhighlight($args) {
...
function book_adminapi_deletehighlight($args) {
// Argument check
    if (!isset($args['id'])) {
        LogUtil::registerArgsError();
        return false;
    ...
    pnModCallHooks('item', 'deletehighlight', $id, '');

// Let the calling process know that we have finished successfully
    return true;
}

function book_adminapi_getemptyglossaryitems($args) {
//security check
...
function book_adminapi_dosearchreplacebook($args) {
...
//grab all the chapters from the book
    $items = pnModAPIFunc('Book', 'user', 'getallchapters', array('book_id' => $args['book_id']));
...
//call the serach and replace function on each chapter
        ...
        $return_text .= book_adminapi_dosearchreplacechap($args);
    }
    return $return_text;
}

function book_adminapi_dosearchreplacechap($args) {
...
// avoid potential security holes or just too much wasted processing
    ...
    $art_items = pnModAPIFunc('Book', 'user', 'getallarticles', array('chap_id' => $args['chap_id']));
    if(!$art_items) {
        LogUtil::registerArgsError();
    }
    $preview_text = "";
...
function book_adminapi_addglossaryitems($args){
    $art_id = $args['art_id'];
    //get the article
    $article = pnModAPIFunc('Book', 'user', 'getarticle', array ('art_id' => $art_id));
     //add glossary terms
    ...
    //save the article
    return pnModAPIFunc('book', 'admin', 'updatearticle', $article);
...
function book_adminapi_add_glossary_terms($args) {
    // Security check - important to do this as early on as possible to
    // avoid potential security holes or just too much wasted processing
    if (!SecurityUtil::checkPermission('Book::', "::", ACCESS_EDIT)) {
        LogUtil::registerPermissionError();
    ...
    $glossary_terms = pnModAPIFunc('Book', 'user', 'getallglossary');
    $terms = array();
    ...
    usort($terms, _sort_term_sizes);
    //now search for all the terms
    $result = preg_replace_callback($terms, _glossreplacecallback, $contents, 1);
    if($result != "") {
        $contents = $result;
    }
 ...
 */
function _glossreplacecallback($matches) {
