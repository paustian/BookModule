 ...
 */
function book_userapi_getall($args) {
    
    //check to see how many books there are
    $numitems = pnModAPIFunc('Book', 'user', 'countitems');
    if($numitems == 0) {
...
function book_userapi_get($args) {
...
function book_userapi_getallchapters($args) {
    $all_chapters = false;
    // Argument check - make sure that all required arguments are present, if
    // not then set an appropriate error message and return
...
function book_userapi_getchapter($args) {
 ...
 *
 */
function book_userapi_getallarticles($args) {
    // Get arguments from argument array
    $chap_id = $args['chap_id'];
...
function book_userapi_getarticle($args) {
    $art_id = $args['art_id'];
    // Argument check - make sure that all required arguments are present, if
    // not then set an appropriate error message and return
    if (!isset($art_id)) {
...
function book_userapi_getarticlebyartnumber($args) {
    // Argument check - make sure that all required arguments are present, if
    // not then set an appropriate error message and return
...
function book_userapi_getallfigures($args) {
    if((!isset($args['book_id']))) {
        $book_id = -1;
    } else {
        $book_id = $args['book_id'];
...
function book_userapi_getallglossary($args) {
    // Argument check - make sure that all required arguments are present, if
    // not then set an appropriate error message and return
...
function book_userapi_findglossaryterm($args) {
    $term = $args['term'];
...
function book_userapi_getglossary($args) {
    // Get arguments from argument array
...
function book_userapi_getfigure($args) {
 ...
 */
function book_userapi_countitems() {
 ...
 */
function book_userapi_countchapters($args) {
 ...
 *
 */
function book_userapi_setcounter($args) {
    $art_id = $args['art_id'];
...
function book_userapi_gethighlights($args) {
...
// Internal callback class used to check permissions to each book item
class book_result_checker {
    ...
    function checkResult(&$item) {
        //print_r($item);die;
        $ok = SecurityUtil::checkPermission( 'Book::Chapter', "$item[book_id]::$item[chap_id]", ACCESS_READ);
        return $ok;
    }
}
?>